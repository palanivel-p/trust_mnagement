...
if(isset($_POST['pay_id']) && isset($_POST['doner_id']))
{
    ...
    if(mysqli_num_rows($resValidateCookie) > 0) {
        ...
        if (mysqli_num_rows($resValidate) > 0 )  {
            ...
            $sqlUpdate = "UPDATE `transaction` SET `verify`='3',`verify_date`='$date' WHERE `pay_id`='$pay_id'";
            mysqli_query($conn, $sqlUpdate);

            $sqlUpdateD = "UPDATE `doner_profile` SET `verfied`='3' WHERE `doner_id`='$doner_id'";
            mysqli_query($conn, $sqlUpdateD);

            $json_array['status'] = "success";
            $json_array['msg'] = "Not Ok successfully";
