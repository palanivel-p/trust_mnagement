   ...
   // $sqlValidateCookie="SELECT * FROM `admin_login` WHERE panel_api_key='$api_key'";
    ...
    if(mysqli_num_rows($resValidateCookie) > 0) {
        ...
        if (mysqli_num_rows($resValidate) > 0 || ($pay_id==$old_pa_id))  {
            ...
            $sqlUpdate = "UPDATE `transaction` SET `pay_id`='$pay_id',`doner_name`='$doner_name',`doner_type`='$doner_type',`dob`='$dob',`donation_date`='$donation_date',`donation_time`='$donation_time',`pan`='$pan',`address`='$address',`mobile`='$mobile',`amount`='$amount',`pay_mode`='$pay_mode',`mobile1`='$mobile1',`remark`='$remark',`transaction_id`='$transaction_id',`type`='$type' WHERE `pay_id`='$old_pa_id'";
